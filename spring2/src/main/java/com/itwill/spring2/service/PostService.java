...
@Slf4j
@Service // -> 스프링 컨테이너에 서비스 컴포넌트로 등록함. 스프링 컨테이너가 PostService 객체 생성하고 필요한 곳에 넣어줌(의존성
			// 주입인가? 그거 해준다고 )
...
//		}
		...
		return list.stream()// 리스트의 스트림. 리스트의 원소들이 하나씩 통과하는 통로->스트림
				.map(PostListDto::fromEntity) // 람다 표현식. map((x) -> PostListDto.fromEntity(x))를 간단히 쓴 것. 아규먼트를 전달하면 해야될일
												// 리턴값
				// 리스트에서 하나씩 하나씩 통과한 게 x. 스트림에 map이 있는데 Post타입의 객체가 지나감 맵을 통과하면 PostListDto로
				// 만들어지는 것
